...
function buildExampleOption(option) {
  return option.setName("topic").setDescription("What example do you want to see?").setRequired(true).addChoices({ name: "Hello World", value: "hello" }, { name: "Variables", value: "variables" }, { name: "Functions", value: "functions" }, { name: "Loops", value: "loops" }, { name: "Conditions", value: "conditions" }, { name: "Testing", value: "testing" }, { name: "Discord Bot", value: "discord" });
}
function buildSyntaxOption(option) {
  return option.setName("keyword").setDescription("Which keyword?").setRequired(true).addChoices({ name: "say (print)", value: "say" }, { name: "is (assign/compare)", value: "is" }, { name: "to (function)", value: "to" }, { name: "if / otherwise", value: "if" }, { name: "for each (loop)", value: "for" }, { name: "test / expect", value: "test" }, { name: "use (import)", value: "use" }, { name: "give back (return)", value: "give back" });
}
let exampleCmd = new SlashCommandBuilder();
...
let syntaxCmd = new SlashCommandBuilder();
...
let commands = [pingCmd.toJSON(), helpCmd.toJSON(), exampleCmd.toJSON(), syntaxCmd.toJSON()];
let rest = new REST({ version: "10" });
rest.setToken(process.env.DISCORD_TOKEN);
let GUILD_ID = "1480524541645295700";
